...
class Bd {
  ...
  consultar() {
    ...
    for (let i = 1; i <= id; i++) {
      ...
      switch (despesaItem.tipo) {
        case "1":
          despesaItem.tipo = "Alimentação";
          break;
        case "2":
          despesaItem.tipo = "Educação";
          break;
        case "3":
          despesaItem.tipo = "Lazer";
          break;
        case "4":
          despesaItem.tipo = "Saúde";
          break;
        case "5":
          despesaItem.tipo = "Transporte";
          break;
      }
  ...
  filtrar(desp) {
    ...
    switch (despFilter.tipo) {
      case "1":
        despFilter.tipo = "Alimentação";
        break;
      case "2":
        despFilter.tipo = "Educação";
        break;
      case "3":
        despFilter.tipo = "Lazer";
        break;
      case "4":
        despFilter.tipo = "Saúde";
        break;
      case "5":
        despFilter.tipo = "Transporte";
        break;
    }
    //config do filter
    if (ano.value != "") {
      despesaFiltroArr = despesaFiltroArr.filter(
        (d) => d.ano == despFilter.ano
      );
    }
    if (mes.value != "") {
      despesaFiltroArr = despesaFiltroArr.filter(
        (d) => d.mes == despFilter.mes
      );
    }
    if (dia.value != "") {
      despesaFiltroArr = despesaFiltroArr.filter(
        (d) => d.dia == despFilter.dia
      );
    }
    if (tipo.value != "") {
      despesaFiltroArr = despesaFiltroArr.filter(
        (d) => d.tipo == despFilter.tipo
      );
    }
    if (desc.value != "") {
      despesaFiltroArr = despesaFiltroArr.filter(
        (d) => d.value == despFilter.value
      );
    }
    if (valor.value != "") {
      despesaFiltroArr = despesaFiltroArr.filter(
        (d) => d.valor == despFilter.valor
      );
    }
    table.innerHTML = "";
